...
## Temporary Makemodel function that will be integrated into pyEXP

def makemodel(func, M, funcargs, rvals = np.logspace(-2.,4.,2000), pfile='', plabel = '', verbose=True):
    """make an EXP-compatible spherical basis function table
    
    inputs
    -------------
    func        : (function) the callable functional form of the density
    M           : (float) the total mass of the model, sets normalizations
    ...
    for i in range(1, dvals.size):
        dr = rvals[i] - rvals[i-1]
        mvals[i] = mvals[i-1] + 2.0*np.pi*(dvals[i-1] * rvals[i-1]**2 + dvals[i] * rvals[i]**2)*dr
        pwvals[i] = pwvals[i-1] + 2.0*np.pi*(rvals[i-1] * dvals[i-1] + rvals[i] * dvals[i])*dr
    
    # evaluate potential (see theory document) -> Update this reference
